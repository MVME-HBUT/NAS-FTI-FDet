...
PRIMITIVES = [
#    'stack_x6_stdConv+stdConv_dilSep_dilSep3+stdConv_dilSep',
#    'stack_x6_sinSepConv+sinSepConv_dilSep_dilSep3+sinSepConv_dilSep',
    'stack_x5_stdConv+stdConv_dilSep+stdConv_dilSep',
    'stack_x5_sinSepConv+sinSepConv_dilSep+sinSepConv_dilSep',
#    'stack_x4_stdConv+stdConv_dilSep+stdConv',
#    'stack_x4_sinSepConv+sinSepConv_dilSep+sinSepConv',
    'def_conv_3x3',
    'none'
]
...
#    print(alpha[3:])
    for i, edges in enumerate(alpha):
        ...
        topk_edge_values, topk_edge_indices = torch.topk(edge_max.view(-1), k) #展平
        node_gene = []
        for edge_idx in topk_edge_indices:
            ...
            node_gene.append((prim, edge_idx.item())) #.item() 将只有一个元素的numpy数组或tensor张量转化为标量的方法
