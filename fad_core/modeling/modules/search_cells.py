    ...
    def __init__(self, n_nodes, C_pp, C_p, C, reduction_p, reduction, bottleNeck=1, norm=True, relu=True):
        ...
        for i in range(self.n_nodes):
            ...
            for j in range(2+i): # include 2 input nodes
                ...
                # op = ops.MixedOp(self.C_node, stride, norm=norm, relu=relu)
                if self.bottleNeck != 1:
                    op_all = nn.ModuleList()
                    op_all.append(ops.GhostModule(self.C_node, C))
                    # op_all.append(nn.Conv2d(self.C_node, C, 1, stride=1, bias=False))
                    op_all.append(op)
                    op_all.append(ops.GhostModule(C, self.C_node))
    ...
    def forward(self, s0, s1, w_dag):
        ...
        for edges, w_list in zip(self.dag, w_dag):
            if self.bottleNeck != 1:
                ...
                #     print(i,s.shape)
                # s_cur = sum(edges[i][1](edges[i][0](s), w) for i, (s, w) in enumerate(zip(states, w_list)))
            else:
