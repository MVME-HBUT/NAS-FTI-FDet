...
@registry.BACKBONES.register("RFDNET")
def build_rfdnet_backbone(cfg):
    body = RFDNet.RFDNet(cfg)
    in_channels_stage2 = [256,512,1024,2048]
    out_channels = cfg.MODEL.RESNETS.BACKBONE_OUT_CHANNELS
    fpn = fpn_module.FPN(
        in_channels_list=[
            0,
...
@registry.BACKBONES.register("RFDNET-FPN")
def build_rfdnet_fpn_backbone(cfg):
    body = RFDNet.RFDNet(cfg)
    in_channels_stage2 = [256,512,1024,2048]
    out_channels = cfg.MODEL.RESNETS.BACKBONE_OUT_CHANNELS
    fpn = fpn_module.FPN(
