...
from fad_core.modeling.rpn.fcos.fad_fcos import build_fad_fcos
from fad_core.modeling.rpn.retinanet.fad_retinanet import build_fad_retinanet 
...
class RPNModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None, timer=None):
        """
        Arguments:
...
def build_rpn(cfg, in_channels):
    ...
    if cfg.MODEL.FAD_ON:
        if cfg.MODEL.FCOS_ON:
            return build_fad_fcos(cfg, in_channels)
        if cfg.MODEL.RETINANET_ON:
            return build_fad_retinanet(cfg, in_channels)    
